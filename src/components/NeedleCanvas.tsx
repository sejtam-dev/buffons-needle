...
const NeedleCanvas = forwardRef<NeedleCanvasHandle, NeedleCanvasProps>(function NeedleCanvas(
    ...
    function drawLines(ctx: CanvasRenderingContext2D) {
        ctx.strokeStyle = isDarkRef.current ? "rgba(148, 163, 184, 0.30)" : "rgba(100, 116, 139, 0.35)";
        ctx.lineWidth = 1;
    ...
    const drawNeedle = useCallback(
        (ctx: CanvasRenderingContext2D, needle: Needle) => {
    ...
    }, [width, height, needleLength, lineSpacing, drawNeedle]);

    // ── incremental append ────────────────────────────────────────────────────

    const appendNeedles = useCallback(
        (batch: Needle[]) => {
            const ctx = getCtx();
            if (!ctx) return;
