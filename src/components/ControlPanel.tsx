    ...
    return (
        <div className={`space-y-1.5 ${disabled ? "opacity-50 pointer-events-none" : ""}`}>
            <div className="flex justify-between items-center">
                ...
                {editing ? (
                    <div
                        className="flex items-center gap-1 rounded-lg px-2 py-0.5 border border-violet-500"
            ...
            {open &&
                ...
                createPortal(
                    ...
                    >
                        ...
                        {/* Formula box */}
                        <div
                            className="mx-3 mb-2 py-2 rounded-lg flex justify-center"
                        ...
                        {/* Explanation */}
                        <p
                            className="text-xs leading-relaxed text-justify px-3 pb-3"
    ...
    return (
        ...
        >
            <span className="text-sm flex items-center gap-0.5" style={{ color: "var(--text-muted)" }}>
                <MathText text={label} />
                {formula && formulaExplanation && <StatTooltip formula={formula} explanation={formulaExplanation} />}
            </span>
            <span
                className={`text-sm font-mono font-semibold ${highlight ? "text-violet-500 text-base" : ""}`}
    ...
    return (
        <div className="pt-2 space-y-2 border-t" style={{ borderColor: "var(--border)" }}>
            ...
            <button
                ...
                className={`w-full py-2.5 rounded-xl border border-violet-500/40 hover:border-violet-400 hover:bg-violet-500/10 text-violet-500 text-sm font-semibold transition-colors ${isMaxReached ? "opacity-40 pointer-events-none" : ""}`}
            >
                {t("btnDropOne")}
            </button>
        </div>
        ...
        >
            ...
            <div className="p-5 flex-1 space-y-5">
                {tab === "params" && (
                    <>
                        <p className="text-xs" style={{ color: "var(--text-subtle)" }}>
                            {t("panelNeedleConstraint")}
                        </p>
                        <SliderField
                            label={t("labelNeedleLength")}
                            value={config.needleLength}
                            min={10}
                            max={config.lineSpacing}
                            step={1}
                            displayValue={`${config.needleLength} px`}
                            displayMin="10 px"
                            displayMax={`${config.lineSpacing} px`}
                            description={t("descNeedleLength")}
                            onChange={(v) => update({ needleLength: v })}
                            disabled={isRunning}
                        />
                        <SliderField
                            ...
                            max={200}
                            step={5}
                            displayValue={`${config.lineSpacing} px`}
                            displayMin="30 px"
                            displayMax="200 px"
                            description={t("descLineSpacing")}
                            onChange={(v) => update({ lineSpacing: v, needleLength: Math.min(config.needleLength, v) })}
                            disabled={isRunning}
                        />
                        <SliderField
                            label={t("labelMaxNeedles")}
                            value={config.maxNeedles}
                            min={100}
                            max={50000}
                            step={100}
                            displayValue={config.maxNeedles.toLocaleString()}
                            displayMin="100"
                            displayMax="50 000"
                            description={t("descMaxNeedles")}
                            onChange={(v) => update({ maxNeedles: v })}
                            disabled={isRunning}
                        />
                    </>
                )}
            ...
            <div className="px-5 pb-3 space-y-1.5 border-t" style={{ borderColor: "var(--border)" }}>
                ...
                <p className="text-xs" style={{ color: "var(--text-subtle)" }}>
                    {formatSpeedDesc(config.speed, t)}
                </p>
            </div>
