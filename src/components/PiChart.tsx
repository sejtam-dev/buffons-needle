    ...
    return (
        ...
        >
            <div className="font-mono font-bold">
                <MathText text={`$\\hat{\\pi}$ ≈ ${val.toFixed(6)}`} />
            </div>
            <div style={{ color: "var(--text-muted)" }}>
                <MathText text={`$\\Delta$ ${err.toFixed(6)}`} />
            </div>
        </div>
    );
}
...
function ChartInner({ history, height, showBrush }: { history: PiDataPoint[]; height: number; showBrush: boolean }) {
    ...
    if (history.length === 0) {
        return (
            <div
                ...
                style={{ height, color: "var(--text-subtle)", fontSize: 13 }}
            >
                {t("chartNoData")}
            </div>
        );
    ...
    return (
        <ResponsiveContainer width="100%" height={height}>
            <LineChart data={history} margin={{ top: 8, right: 24, bottom: showBrush ? 8 : 4, left: 8 }}>
                ...
                />
                {showBrush && (
                    <Brush dataKey="total" height={22} stroke={lineColor} fill={brushBg} travellerWidth={6} />
                )}
            </LineChart>
        </ResponsiveContainer>
    );
